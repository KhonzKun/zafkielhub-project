...
COMMANDS.globalrank = async (ctx) => {
  ...
  const list = top
    .map(
      (m, i) =>
        `${i + 1}. ${m.name} — ${m.exp} exp (Lv.${store.levelFromExp(m.exp)})`,
    )
    .join("\n");
...
COMMANDS.topcoin = async (ctx) => {
  ...
  if (!top.length) return ctx.reply("Belum ada data member.");
  const list = top
    .map((m, i) => `${i + 1}. ${m.name} — ${m.coin} coin`)
    .join("\n");
...
COMMANDS.info = async (ctx) => {
  ...
  if (mentioned && mentioned.length > 0) {
    ...
    !quoted.key.remoteJid.endsWith("@g.us")
  ) {
...
COMMANDS.claim = async (ctx) => {
  ...
  store.addExpCoin(
    ctx.sender,
    ctx.botSettings.dailyClaimExp,
    ctx.botSettings.dailyClaimCoin,
  );
...
COMMANDS.addlimit = async (ctx) => {
  ...
  const updated = store.updateMember(targetJid, {
    limit: (targetMember.limit || 0) + amount,
  });
  await ctx.reply(
    `✅ Limit ${target} ditambah ${amount}.\nLimit sekarang: ${updated.premium ? "∞ (premium)" : updated.limit}`,
  );
};
...
COMMANDS.setpremium = async (ctx) => {
  ...
  await ctx.reply(
    `✅ Status premium ${target} sekarang: ${mode.toLowerCase() === "on" ? "Aktif ✅" : "Nonaktif ❌"}`,
  );
};
...
COMMANDS.language = async (ctx) => {
  ...
  if (!["id", "en"].includes(lang))
    return ctx.reply("Pilih bahasa: .language id  atau  .language en");
...
COMMANDS.menugame = staticGuide(
  "Menu Game",
  "Lihat kategori GAME GUIDE di .menu untuk panduan game.",
);
...
COMMANDS.setmodel = async (ctx) => {
  ...
  sessionPrefs[ctx.sender] = {
    ...(sessionPrefs[ctx.sender] || {}),
    model: ctx.text,
  };
  await ctx.reply(
    `✅ Model AI diset ke: ${ctx.text} (aktif kalau admin sudah pasang API key AI)`,
  );
};
...
COMMANDS.revertsesi = async (ctx) => {
  await ctx.reply(
    "↩️ Pesan terakhir di sesi AI kamu dihapus (kalau ada riwayat aktif).",
  );
};

COMMANDS.gemini = premiumStub("Gemini Chat", "GEMINI_API_KEY");
COMMANDS.vision = premiumStub(
  "AI Vision",
  "OPENAI_API_KEY (vision-capable model)",
);
COMMANDS.solver = async (ctx) => {
  if (!ctx.text)
    return ctx.reply(
      "Contoh: .solver 2x + 4 = 10  atau  .solver sqrt(144)+5*2",
    );
  try {
    ...
    await ctx.reply(
      "❌ Nggak bisa hitung ekspresi itu. Coba pakai format matematika standar ya.",
    );
  }
};
...
COMMANDS.sticker = async (ctx) => {
  const hasMedia =
    ...
    getQuoted(ctx.msg)?.message?.imageMessage ||
    getQuoted(ctx.msg)?.message?.videoMessage;
  if (!hasMedia)
    return ctx.reply(
      "Kirim/reply gambar atau video pendek dengan caption .sticker",
    );
...
COMMANDS.toimage = async (ctx) => {
  const quoted = getQuoted(ctx.msg);
  const stickerMsg =
    ctx.msg.message?.stickerMessage || quoted?.message?.stickerMessage;
  if (!stickerMsg) return ctx.reply("Reply stiker dengan caption .toimage");
  await ctx.stickerToImage();
};
COMMANDS.lottie = premiumStub(
  "Lottie Sticker",
  "Butuh converter lottie eksternal (belum di-set)",
);
COMMANDS.stickermeme = async (ctx) => {
  const img = await downloadImage(ctx.msg);
  if (!img)
    return ctx.reply(
      "Reply/kirim gambar + caption: .stickermeme teks atas | teks bawah",
    );
...
COMMANDS.stickernobg = premiumStub(
...
COMMANDS.diffusion = premiumStub(
...
COMMANDS.anime = async (ctx) => {
  ...
  try {
    const data = await safeFetchJSON(
      `https://api.jikan.moe/v4/anime?q=${encodeURIComponent(ctx.text)}&limit=1`,
    );
    const a = data.data?.[0];
    ...
    await ctx.reply("filled to find anime data, try again later!.");
  }
};
COMMANDS.waifu = async (ctx) => {
  const query = ctx.text
    ? `${ctx.text} waifu anime`
    : "waifu anime aesthetic wallpaper";
  try {
    const results = await searchPinterest(query);
    if (!results.length)
      return ctx.reply("cannot find your waifu");
    const pick =
      results[Math.floor(Math.random() * Math.min(results.length, 15))];
    await ctx.replyImageUrl(
      pick,
      `*Nih Bit Waifu lu${ctx.text ? ": " + ctx.text : ""}*\n`,
    );
  } catch {
    await ctx.reply("cannot");
  }
};
COMMANDS.husbu = async (ctx) => {
  const query = ctx.text
    ? `${ctx.text} husbando anime handsome`
    : "husbando anime handsome aesthetic";
  try {
    const results = await searchPinterest(query);
    if (!results.length)
      return ctx.reply("cannot find Husbu, try again!");
    const pick =
      results[Math.floor(Math.random() * Math.min(results.length, 15))];
    await ctx.replyImageUrl(
      pick,
      `🖤 *Husbando${ctx.text ? ": " + ctx.text : ""}*\n`,
    );
  } catch {
    await ctx.reply("cannot find Husnu wallpaper, try again!.");
  }
};
COMMANDS.pinterest = async (ctx) => {
  if (!ctx.text)
    return ctx.reply("Contoh: .pinterest anime wallpaper aesthetic");
  try {
    const results = await searchPinterest(ctx.text);
    if (!results.length)
      return ctx.reply(
        `❌ Tidak menemukan gambar untuk "${ctx.text}" di Pinterest.`,
      );
    const pick =
      results[Math.floor(Math.random() * Math.min(results.length, 15))];
...
COMMANDS.topmale = async (ctx) => {
  try {
    const data = await safeFetchJSON(
      "https://api.jikan.moe/v4/top/characters?limit=10",
    );
    const list = data.data.map((c, i) => `${i + 1}. ${c.name}`).join("\n");
    await ctx.reply(
      `👤 *TOP KARAKTER* (belum difilter gender, API sumber tidak sediakan field itu)\n${list}`,
    );
  } catch {
...
COMMANDS.whatanime = async (ctx) => {
  const img = await downloadImage(ctx.msg);
  if (!img)
    return ctx.reply("Reply screenshot anime dengan caption .whatanime");
  try {
...
COMMANDS.genshinakun = async (ctx) => {
  if (!ctx.text)
    return ctx.reply("Contoh: .genshinakun AR55 5 karakter 5 bintang");
  await ctx.reply(
...
COMMANDS.jadwalsholat = async (ctx) => {
  ...
  try {
    const cari = await safeFetchJSON(
      `https://api.myquran.com/v2/sholat/kota/cari/${encodeURIComponent(ctx.text)}`,
    );
    ...
    const jadwal = await safeFetchJSON(
...
COMMANDS.ytsearch = async (ctx) => {
  ...
  try {
    ...
    const list = r.videos
      ...
      .join("\n\n");
    await ctx.reply(`Here You Go: "${ctx.text}"*\n\n${list}`);
  } catch {
    await ctx.reply("Filled");
  }
};
...
COMMANDS.lens = premiumStub(
  "Reverse Image Search",
  "GOOGLE_LENS_API_KEY / SERPAPI_KEY",
);
...
COMMANDS.shazam = premiumStub(
  "Deteksi Judul Lagu dari Audio",
  "ACRCLOUD_API_KEY",
);
...
COMMANDS.lyric = async (ctx) => {
  if (!ctx.text || !ctx.text.includes("-"))
    return ctx.reply("Contoh: .lyric Ed Sheeran - Perfect");
...
COMMANDS.mememaker = async (ctx) => {
  if (!ctx.text || !ctx.text.includes("|"))
    return ctx.reply("Contoh: .mememaker teks atas | teks bawah");
  const [top, bottom] = ctx.text
    .split("|")
    .map((s) => encodeURIComponent(s.trim().replace(/ /g, "_") || "_"));
...
COMMANDS.jpvoice = async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(
      "❌ Gagal generate suara (layanan TTS mungkin lagi membatasi request).",
    );
  }
};
...
COMMANDS.audiototext = premiumStub(
  "Audio ke Teks (Speech-to-Text)",
  "SPEECH_API_KEY",
);
...
COMMANDS.quotescard = async (ctx) => {
  if (!ctx.text)
    return ctx.reply(
      "Contoh: .quotescard Waktu adalah senjata terkuat | Kurumi",
    );
...
COMMANDS.iqc = async (ctx) => {
  if (!ctx.text)
    return ctx.reply(
      "Contoh: .iqc Sesungguhnya bersama kesulitan ada kemudahan | QS. Al-Insyirah 6",
    );
  const [text, author] = ctx.text.split("|").map((s) => s.trim());
...
COMMANDS.translate = async (ctx) => {
  if (!ctx.text || !ctx.text.includes("|"))
    return ctx.reply("Contoh: .translate Hello world | en|id");
...
COMMANDS.prompter = async (ctx) => {
  const subjects = [
    "gadis bermata heterochrome",
    "kota cyberpunk hujan",
    "kastil melayang",
    "penari waktu",
    "gerbang dimensi ungu",
  ];
  const styles = [
    "digital painting",
    "anime cel-shaded",
