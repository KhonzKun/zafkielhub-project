...
const { buildMenuCaption, buildMenuBanner, findCommandMeta } = require("./menu");
...
function attachCommandHandler(sock, logActivity) {
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    // CATATAN: pesan fromMe (dikirim dari akun/nomor bot itu sendiri, termasuk
    // chat "Kirim pesan ke diri sendiri") SENGAJA TETAP DIPROSES di sini.
    // Ini aman dari looping karena command cuma dijalankan kalau teksnya
    // diawali prefix (. ! /) DAN cocok sama nama command yang ada - balasan
    // bot sendiri (mis. "🏓 Pong!", teks caption .menu) nggak pernah diawali
    // prefix itu, jadi otomatis diabaikan (lihat parsePrefixCommand di bawah).
    ...
    logActivity(`Pesan masuk dari ${jidToNumber(sender)}${parsed ? ` (command: ${parsed.cmd})` : ""}`);

    if (!parsed) return; // bukan command, biarkan aja (bisa dikembangkan jadi chat AI bebas kalau mau)

    const member = store.getMember(sender, pushName);
    const level = store.levelFromExp(member.exp);

    // .menu / !menu -> kirim banner + caption menu lengkap
    if (parsed.cmd === "menu" || parsed.cmd === "help") {
      try {
        ...
        await sock.sendMessage(jid, { image: banner, caption }, { quoted: msg });
        store.addExpCoin(sender, 1, 0);
      } catch (err) {
        logActivity(`Gagal kirim menu: ${err.message}`);
        await sock.sendMessage(jid, { text: "[-] Gagal membuat menu, silakan coba lagi." }, { quoted: msg });
      }
      return;
    }

    const handler = COMMANDS[parsed.cmd];
    if (!handler) return; // command nggak dikenal, diemin aja biar nggak spam grup
    ...
    // ctx yang dikirim ke setiap command handler
    const ctx = {
      sock,
      msg,
      jid,
      ...
      replyImageUrl: async (url, caption) => {
        if (!url) return sock.sendMessage(jid, { text: caption || "Gagal ambil gambar." }, { quoted: msg });
        await sock.sendMessage(jid, { image: { url }, caption: caption || "" }, { quoted: msg });
      },
      ...
      makeSticker: async () => {
        ...
        if (!buf) return sock.sendMessage(jid, { text: "Gagal ambil media." }, { quoted: msg });
      ...
      stickerToImage: async () => {
        ...
        const stickerMsg = msg.message?.stickerMessage || quoted?.message?.stickerMessage;
    ...
    try {
      ...
      await sock.sendMessage(jid, { text: `[-] Terjadi error saat menjalankan .${parsed.cmd}` }, { quoted: msg });
    }
  });
}
